...
MusicGrabber включает Telegram бота для поиска и загрузки музыки прямо из мессенджера.
...
2. **Добавьте в docker-compose.yml**
   ...
   environment:
     ...
     # Опционально: ограничить доступ конкретным пользователям
...
- 🔍 **Поиск** по всем источникам (YouTube, SoundCloud, Monochrome)
- 🎵 **Inline кнопки** для выбора трека из результатов
- ⚙️ **Настройки** прямо в боте (источник, качество, папка)
- 📋 **Очередь** с отслеживанием статуса загрузки
- 📊 **Статистика** загрузок
- 🎼 **FLAC качество** по умолчанию

### Использование

1. Откройте диалог с ботом
2. Нажмите `/start` для главного меню
3. Выберите "🎵 Поиск музыки"
4. Введите название трека
5. Выберите результат из списка
6. Файл автоматически загрузится и придёт в чат

### Ограничение доступа

Чтобы ограничить доступ только определённым пользователям:

1. Узнайте ваш chat ID: @userinfobot или `/whoami` в боте
2. Добавьте в docker-compose.yml:
   ```yaml
   - TELEGRAM_ALLOWED_USERS=123456789,987654321
   ```
...
Самый простой способ настройки — через `.env` файл:
...
3. **Добавьте токен бота**
   ```env
   TELEGRAM_BOT_TOKEN=8618035807:AAHYJEJUg-qAQvH5ZFjSvOw7CzGO8wTVFAw
   ```
...
- По умолчанию **все пользователи** имеют доступ
- Для ограничения доступа добавьте chat ID:
   ```env
   TELEGRAM_ALLOWED_USERS=123456789,987654321
   ```
- Узнайте ваш chat ID: @userinfobot
...
Для автоматического тестирования функционала:

```bash
# Запустить e2e тесты
docker exec music-grabber python test_e2e.py

# С тестовым chat ID
TEST_CHAT_ID=123456789 docker exec music-grabber python test_e2e.py
```
...
1. **Проверка поиска:**
   ```
   /start → 🎵 Поиск музыки → Arctic Monkeys
   ```

2. **Проверка существующего файла:**
   ```
   Поиск: DJ Snark
   Выберите трек "Это вайб пати, детка"
   Должен прийти ответ: "Уже загружен"
   ```

3. **Проверка загрузки:**
   ```
   Поиск: любой новый трек
   Нажмите на результат
   Проверьте /queue — статус должен быть "queued"
   ```
